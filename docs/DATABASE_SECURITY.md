...
Phase 7B Media attachment rows contain required accessible text but no URL, source path, internal label, file bytes, actor profile, or request payload. Foreign keys prevent attachment to nonexistent Pages/assets. A transaction verifies the draft status and exact lock version before relation changes, increments the Page lock, and records the actor/request through the bounded content audit. Public delivery requires a fresh indexed join to at least one published Page, serves only the sanitized preview derivative, and logs failures without the requested asset ID or storage path. Public disclosure is not revocable after clients receive bytes.

Phase 7C lifecycle operations add no schema. Usage reporting is an administrator-only bounded aggregate query. Deletion locks and rechecks the catalog relationship count while the existing foreign key remains the final concurrent-attachment guard. Regeneration verifies the master SHA-256 before atomically replacing the preview. Controlled events contain no file bytes, paths, labels, Page titles, or payloads.
